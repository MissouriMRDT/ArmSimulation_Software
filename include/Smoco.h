...
private:
    ...
    int16_t m_dutyCycle = 0;               // (1/32768)
    float m_errorGain = 0;
    int32_t m_targetPosition = 0; // (step)
    int32_t m_targetVelocity = 0; // (step/s)
    float m_targetCurrent = 0;    // (A)
    double m_rampRate = 0;        // (1/s)
    struct PID m_PID = { 0 };
    int32_t m_softLimitAPosition = 0;   // (step)
    int32_t m_softLimitBPosition = 0;   // (step)
    int16_t m_calibrationDutyCycle = 0; // (1/32768)
    int32_t m_calibrationPosition = 0;  // (step)
    bool m_debugTelemetryEnabled = 0;   // true: SMoCo will continuously send debug
                                        // telemetry

    // true: received Position Calibrated message and calibratePosition has not been called since
    bool m_calibrated = false;
    bool m_limitSwitchA = false; // true: limit switch A depressed
    ...
    uint64_t m_echoResponse = 0;       // payload of most recent received Echo Reply
...
public:
    ...
    Smoco(int32_t initialPosition, float maxSpeed);
